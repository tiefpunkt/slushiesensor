...
byte probe01[8] = { 
  0x28, 0x09, 0xD9, 0xDD, 0x04, 0x00, 0x00, 0x27 }; 
byte probe02[8] = { 
  0x28, 0x5D, 0x78, 0xDE, 0x04, 0x00, 0x00, 0xA3 };
byte probe03[8] = { 
  0x28, 0x50, 0x05, 0xDE, 0x04, 0x00, 0x00, 0x68 };
...
void loop() {
  ...
  checkButtons();
  updateDisplay();
  float t1 = getTemp(probe01);
  float t2 = getTemp(probe02);
  float t3 = getTemp(probe03);
  if (Serial) {
    Serial.print(t1,1);
    Serial.print(";");
    Serial.print(t2,1);
